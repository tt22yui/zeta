...
- **工程改进**：GitHub Actions 发布工作流增加 npm 依赖缓存（`setup-node` 配置 `cache: npm`），复用 `package-lock.json` 缓存 `node_modules`，缩短 CI 安装耗时。

- **文档**：AGENTS.md 新增「发布日志」章节，沉淀各版本发布记录。

### v0.1.1（2026-09-02）

- **新增功能**：地点栏交互增强——面包屑子目录下拉、历史下拉键盘遍历（↑/↓/Home/End/Enter/Delete/Esc）、`~` 主目录展开、驱动器下拉面板。

- **Bug 修复**：移除多个标签时累积每次改名后的新路径，避免复用旧文件名导致「找不到文件」。

- **工程改进**：重构 `History` 撤销/重做栈为可独立测试的纯逻辑，新增标签解析单元测试；命令 `#[tauri::command(async)]` 化，避免阻塞 UI 主线程。
