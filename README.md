...
- **拖拽到外部应用**：`Alt` + 拖拽携带真实文件句柄，可直接拖进系统资源管理器 / 飞书 / 企微等（对外语义是复制）。
- **拖到文件夹即剪切**：普通拖拽到列表中的文件夹行上松手，即移动到该文件夹。拖拽期间所有文件夹行标出「可放置」，光标所在行**即时**高亮并显示「松手移入此文件夹」（走浏览器原生拖拽，自带边缘自动滚动），松手后该行闪一下确认；同名项自动加序号、双方都保留。
