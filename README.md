...
- **完整的键盘导航**：与系统资源管理器一致的交互体验。
  ...
  - `F2` 行内重命名 · `F5` 刷新 · `Delete` 删除到回收站 · `Ctrl+Z` / `Ctrl+Shift+Z` 撤销 / 重做
  - 直接敲字符按名称前缀定位
- **空格预览**：图片 / 视频 / 音频（带频谱）/ PDF / Markdown 富文本 / 文本（前 1 MiB），键盘移动光标时预览跟随。
- **文件夹收纳**：解散文件夹（子项上移到上级 + 删除空壳）与收入文件夹（新建文件夹收纳选中项）。
- **删除与撤销/重做**：`Delete` 删除到系统回收站（网络路径永久删除前二次确认，批量显示进度）；`Ctrl+Z` / `Ctrl+Shift+Z` 撤销 / 重做打标签、重命名、解散/收入文件夹与拖拽移动，右键菜单亦有「删除」，工具栏按钮按可用态置灰。
- **地址栏即导航**：可直接输入路径（支持 `~` 展开主目录），并保留访问历史与收藏路径。
- **设置面板**：主题三态（跟随系统 / 浅色 / 深色）、地址栏历史条数、启动恢复上次路径、标签分隔符。
- **拖拽即剪切，拖出即复制**：同一个普通拖拽手势，按**落在哪**区分功能 —— 松手在列表里的文件夹行上＝移动到该文件夹（同名项自动加序号、双方都保留）；松手在窗口外（资源管理器 / 飞书 / 企微等）＝复制给对方，携带真实文件句柄。拖拽期间所有文件夹行标出「可放置」，落点行高亮并显示「松手移入此文件夹」，松手后该行闪一下确认。
...
> 说明：删除走系统回收站、**不进入撤销栈**（可自行从回收站恢复）；网络路径无回收站，改为永久删除并二次确认。撤销/重做仅覆盖本应用发起的改名与文件夹收纳/解散/拖拽移动。
...
├── src/                     # React 前端
│   ├── App.tsx              # 主视图与编排
│   ├── components/          # 标题栏 / 工具栏 / 地址栏 / 文件表 / 标签栏 / 状态栏
│   ├── hooks/               # 选中、地址栏、拖放、撤销等自定义 hook
│   ├── api.ts               # Tauri 命令封装（invoke）
│   ├── types.ts             # 前后端共享类型
│   └── styles.css           # 全局样式
├── src-tauri/src/           # Rust 后端（按模块拆分）
│   ├── lib.rs               # 入口：插件注册 + 命令表
│   ├── tags.rs              # 标签解析/规范化 + 分隔符配置
│   ├── fs_util.rs           # 文件系统工具（隐藏/系统判定、原子移动与回滚）
│   ├── history.rs           # 撤销/重做栈
│   ├── browse.rs            # 目录列举、盘符/主目录
│   ├── file_ops.rs          # 打/删标签、重命名、删除
│   ├── folder_ops.rs        # 解散/收入文件夹、拖拽移动
│   ├── preview.rs           # 文本预览
│   └── window.rs            # 启动期窗口 setup
